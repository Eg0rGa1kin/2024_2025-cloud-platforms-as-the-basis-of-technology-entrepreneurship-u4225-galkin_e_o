...
1. Исходные данные о приложении  
Приложение EasyBooking представляет собой веб-сервис, который позволяет арендодателям сдавать квартиры дистанционно. Основные функциональные требования включают:  
...
Для проектирования инфраструктуры мы разработали схему инфраструктуры (https://drive.google.com/file/d/1Hg7OYJJpTgdI4ccRIQQF-gOCOYjjwD3w/view)  
<image src="/Снимок экрана 2024-11-01 в 16.01.58.png">  
...
На начальном этапе приложение должно обрабатывать ограниченное количество запросов, состав инфраструктуры:  
  - Веб-сервер: 1 экземпляр (Google Cloud Compute Engine f1-micro) для обработки запросов от арендодателей и арендаторов.  (upd: был изменен веб-сервер с типа f1-micro на e2-micro. Это изменение было произведено для того, чтобы увеличить производительность веб-сервера без значительного увеличения затрат. Тип f1-micro является самым маленьким и самым дешевым вариантом виртуальной машины в Google Cloud, но он имеет ограниченную вычислительную мощность и память, что может привести к снижению производительности приложения при увеличении нагрузки. В свою очередь, тип e2-micro предлагает более высокую производительность по сравнению с f1-micro, с большим количеством процессорных ядер и памяти, что позволяет обрабатывать больше запросов и обеспечивать лучшую производительность приложения. Несмотря на то, что тип e2-micro немного дороже, чем f1-micro, это увеличение затрат незначительно и компенсируется увеличением производительности и надежности приложения.  
